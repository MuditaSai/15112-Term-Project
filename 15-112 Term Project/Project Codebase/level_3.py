...
def keyPressed(app, event):
    ...
    if not app.isPaused:
        ...
        if app.enemyCurrNode == currPlayerNode:
            app.score -= 15
...
def drawFood(app, canvas):
    for row, col in app.foodLst:
        ...
        cx = (x0 + x1) // 2
        cy = (y0 + y1) // 2
        canvas.create_oval(cx-app.foodR, cy-app.foodR, cx+app.foodR, cy+app.foodR, fill = app.setColor)
...
def drawLevel(app, canvas):
    canvas.create_text(app.width/2, 30, text = "Level: 3", font = 'Phosphate 30 bold')
